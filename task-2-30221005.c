 ...
 *               facilitates the act of Round-Robin process scheduling
 ...
 * Round-Robin process scheduling through calls to other functions
 ...
 * perform the scheduling through the rr_scheduler() function, write the results
...
int main(int argc, char *argv[])
{
	...
	// arranges the processes in process_list into the queue based on arrival time
	// of the processes.
	addToQueue(queue, process_list, num_processes);
	
	// runs the scheduler on the updated list of processes
	rr_scheduler(queue, num_processes);
	
	// write the required results onto an output text file. Exits the program
	// if any error occurs.
	if (writeResults(queue, num_processes) == 1) {
